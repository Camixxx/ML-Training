...
'''
def rnn_unroll(num_rnn_layer, seq_len, num_hidden,  num_label, dropout=0., batch_norm=False):
    ...
    # 将生成的的...个hidden，连接起来，并将他们和num_label个异常单元进行fully
    # connected, 即矩阵相乘，这里每一个hidden都是一个128维的向量
        concat = mx.sym.Concat(*last_hidden, dim=0)

    # 这里的最后一层的隐藏层有10000个，它的权重矩阵是10000X200
    # 表示的是最后的隐藏层到输出层之后的矩阵相乘
    fc = mx.sym.FullyConnected(data=concat, weight=cls_weight, bias=cls_bias,
                                   num_hidden=num_label)
    sm = mx.sym.SoftmaxOutput(data=fc, label=mx.sym.Variable('label'),
                                  name='sm')
    #loss_all.append(sm)
    ...
    for i in range(num_rnn_layer):
        state = last_states[i]
        state = RNNState(h=mx.sym.BlockGrad(state.h, name="l%d_last_h" % i))
        last_states[i] = state

    out_prob = [sm]
    unpack_h = [state.h for state in last_states]
    list_all = out_prob + unpack_h
...
def setup_rnn_model(ctx,
                    num_rnn_layer, seq_len, # seq_len=35, num_lstm_layer=2
                    num_hidden, num_label, # num_hidden=200, num_embed=200,
    ...
    """
    rnn_sym = rnn_unroll(num_rnn_layer=num_rnn_layer, seq_len=seq_len,
    ...
    init_states = [RNNState(h=arg_dict["l%d_init_h" % i]) for i in range(num_rnn_layer)]
    seq_labels = rnn_exec.arg_dict["label"]
    seq_data = [rnn_exec.arg_dict["t%d_data" % i] for i in range(seq_len)]

    last_states = [RNNState( h=out_dict["l%d_last_h_output" % i]) for i in range(num_rnn_layer)]
...
def set_rnn_inputs(m, X, begin):
    '''
    这里是为epoch即一个seq_num=35设置输入输出，输入都是一个128维的向量即为
...
def train_rnn(model, X_train_batch, X_val_batch,
