...
def rnn(num_hidden, in_data, prev_state, param, seqidx, layeridx, dropout=0., batch_norm=False):
    ...
    h2s = mx.symbol.FullyConnected(data=hidden, name='h2s', num_hidden=num_hidden)
    softoutput = mx.symbol.SoftmaxOutput(data=h2s, name='softmax')

    return RNNState(h=softoutput)
